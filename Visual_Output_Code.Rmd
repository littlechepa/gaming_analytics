---
title: "Youzu Game Analytics"
author: "Venkateswarlu Allacheruvu"
date: "April 20, 2018"
output:
...
```
##**Question-1**
**From the given Dataset, determine and analyze Day 1, Day 3, Day 7, Day 15 and Day 30 Retention. (Refer to Session start tab)**



Day N retention also known as classic retention or Retention by Day, is the percent of new users who come back on a specific day.
...
Day 1, Day 3, Day 7, Day 15 and Day 30 Retention rates are as follows
...
**Visualization of the classic retention rate or N Day retention rate**
...
## **Question-2**
**Create a Funnel of level progression and determine if any areas of the game are acting as blockers. (Refer to Level Start tab). Also, identify how many times levels are replayed.**
...
ggplot(level_progression, aes(x = level, y = progression_rate)) +
  ...
  scale_x_continuous(limits = c(0,100), breaks =  seq(0,100,3), expand = c(0,0)) +
  scale_y_continuous(limits = c(0,1), breaks =  seq(0,1,0.05), expand = c(0,0)) +
  theme_light()

```

From the chart progression funnel, it is clearly observed that the levels 10, 16, 23 are seeming like level blockers.
...
ggplot(session_stat_of_every_level_per_user, aes(x = level_id, y = max_sessions )) +
  ...
  geom_smooth() +
  scale_x_continuous(limits = c(0,100), breaks =  seq(0,100,3))+
  theme_light()
```
...
purchase rate of  users, who make first time purchases between levels 1 to 7 is 
...
##**Question-4**
**What percentage of users make a repeat purchase? At what level maximum repeat purchases happen? (Refer to Spenders tab)**



**Here is the total percentage of users make a repeat purchase**
...
These are the levels at which maximum repeat purchases happened. These are the actual levels, at which maximum users repeated purchases rather than very few users.
...
Total revenue is in terms of USD. I have taken care of currency convertion from one currency to other
```{r echo=FALSE}
